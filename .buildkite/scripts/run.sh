...
echo "📦 Installing dependencies (7-zip, aria2, Python3)..."
if command -v sudo >/dev/null 2>&1; then
    sudo apt update
    sudo apt install -y p7zip-full aria2 python3 python3-requests python3-pip curl
else
    apt update
    apt install -y p7zip-full aria2 python3 python3-requests python3-pip curl
fi
...
echo "🚀 Starting concurrent aria2c downloads with timeout handling & completion tracker..."
python3 - << 'EOF'
import os
import glob
import asyncio
...
echo "📦 Running Smart Auto-Group Zipping & Splitting..."
python3 - << 'EOF'
